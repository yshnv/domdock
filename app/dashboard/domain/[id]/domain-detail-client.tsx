...
export default function DomainDetailClient({
  ...
  const deleteDomain = async () => {
    if (
      !confirm(`Are you sure you want to remove ${domain.name} from DomDock?`)
    )
      return;
    setDeleting(true);
      ...
      <header className="sticky top-0 z-40 border-b border-border/30 bg-background/90 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-[1280px] items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-2 min-w-0">
            <Link
              href="/dashboard"
              className="flex items-center gap-2 font-heading text-sm font-bold text-foreground shrink-0"
            >
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        ...
        <div className="mb-6 rounded-2xl border border-[#3139fb]/20 bg-[#fffcec] p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:shrink-0">
              ...
              {monitoring?.ssl_valid !== null &&
                monitoring?.ssl_valid !== undefined && (
                  <StatusPill
                    ok={monitoring.ssl_valid}
                    label={monitoring.ssl_valid ? "SSL Valid" : "SSL Invalid"}
                  />
                )}
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <MiniCard
                ...
                sub={
                  monitoring?.registrar_iana_id
                    ? `IANA ${monitoring.registrar_iana_id}`
                    : "RDAP"
                }
              />
              <MiniCard
              ...
              <MiniCard
                ...
                value={
                  monitoring?.ssl_valid
                    ? `${monitoring.ssl_days_remaining ?? "?"}d remaining`
                    : "Invalid / None"
                }
                sub={
                  monitoring?.ssl_issuer
                    ? monitoring.ssl_issuer
                    : "TLS Port 443"
                }
              />
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Section
                ...
                icon={<Globe className="size-4 text-[#3139fb]" />}
              >
                ...
                <Row
                  ...
                  value={
                    domain.expires_at
                      ? `${domain.expires_at} (${days !== null ? `${days}d` : "—"})`
                      : "Unavailable"
                  }
                />
                <Row
                  label="Registrar"
                  value={monitoring?.registrar_name || "Unavailable"}
                />
                <Row
                  label="IANA ID"
                  value={monitoring?.registrar_iana_id || "—"}
                />
                  ...
                  <Row
                    ...
                    value={
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[#3139fb] hover:underline break-all"
                      >
                        {monitoring.registrar_url}
                      </a>
                    }
              ...
              >
                ...
                <Row
                  label="Final URL"
                  value={
                    monitoring?.website_final_url || `https://${domain.name}`
                  }
                  mono
                />
                <Row
                  label="Redirects"
                  value={String(monitoring?.website_redirect_count ?? 0)}
                />
                <Row
                  label="HTTPS"
                  value={
                    monitoring?.https_available ? "Available" : "Not Available"
                  }
                />
              </Section>
            </div>
        ...
        {activeTab === "dns" && (
          <div className="space-y-4">
            {/* Nameservers */}
            <Section
              title={`Nameservers (${dns.ns?.length || 0})`}
              icon={<Network className="size-4 text-purple-500" />}
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {dns.ns?.length > 0 ? (
                  dns.ns.map((ns) => (
                    <span
                      key={ns}
                      className="rounded-lg border border-purple-500/20 bg-purple-500/10 px-2.5 py-1 font-mono text-xs text-purple-600"
                    >
                      {ns}
                    </span>
                  ))
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              <Row
                label="Status"
                value={
                  monitoring?.ssl_valid ? "✅ Valid" : "❌ Invalid / Expired"
                }
              />
              <Row
                label="Days Remaining"
                value={
                  monitoring?.ssl_days_remaining !== null
                    ? `${monitoring?.ssl_days_remaining} days`
                    : "—"
                }
              />
              ...
              <Row
                label="Subject"
                value={monitoring?.ssl_subject || "—"}
                mono
              />
              <Row
                label="Hostname Match"
                value={monitoring?.ssl_hostname_matches ? "✅ Yes" : "❌ No"}
              />
            </Section>
            <Section
            ...
            >
              <Row
                label="Valid From"
                value={
                  monitoring?.ssl_valid_from
                    ? new Date(monitoring.ssl_valid_from).toLocaleDateString()
                    : "N/A"
                }
              />
              <Row
                label="Valid Until"
                value={
                  monitoring?.ssl_valid_to
                    ? new Date(monitoring.ssl_valid_to).toLocaleDateString()
                    : "N/A"
                }
              />
              <Row
                label="Serial Number"
                value={monitoring?.ssl_serial_number || "N/A"}
                mono
              />
              <Row
                label="HTTPS Available"
                value={monitoring?.https_available ? "✅ Yes" : "❌ No"}
              />
            </Section>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <EmailPill
                ...
                ok={
                  !!(
                    monitoring?.email_dkim_records &&
                    monitoring.email_dkim_records.length > 0
                  )
                }
                info="best effort"
              />
            ...
            <Section
              title="Email Security Details"
              icon={<Mail className="size-4 text-[#3139fb]" />}
              sub="MX, SPF, DMARC, DKIM — checked from DNS records"
            >
              <Row
                label="Mail Exchanger (MX)"
                value={
                  monitoring?.email_has_mx
                ...
                <div className="pb-3 pt-1">
                  ...
                  <div className="space-y-1">
                    {dns.mx.map((m) => (
                      <div
                        key={m.exchange}
                        className="flex items-center justify-between gap-2 rounded-lg bg-muted/40 px-2.5 py-1.5 font-mono text-xs"
                      >
                        <span className="text-muted-foreground w-8 shrink-0">
                          P{m.priority}
                        </span>
                        <span className="break-all text-foreground flex-1">
                          {m.exchange}
                        </span>
                      </div>
                    ))}
              ...
              <Row
                label="SPF Record"
                value={monitoring?.email_spf_record ? "✅ Found" : "⚠️ Missing"}
              />
              {monitoring?.email_spf_record && (
                <RecordBox
                  value={monitoring.email_spf_record}
                  onCopy={copyToClipboard}
                  copied={copiedRecord}
                />
              )}
              ...
              {monitoring?.email_dmarc_record && (
                <RecordBox
                  value={monitoring.email_dmarc_record}
                  onCopy={copyToClipboard}
                  copied={copiedRecord}
                />
              )}
              <Row
                ...
                value={
                  monitoring?.email_dkim_records?.length
                    ? `✅ Found (${monitoring.email_dkim_records.length} record${monitoring.email_dkim_records.length > 1 ? "s" : ""})`
                    : "— Not found"
                }
              />
            </Section>
        ...
        {activeTab === "seo" && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 rounded-2xl border border-[#3139fb]/20 bg-[#fffcec] p-5">
              <div
                className={`grid size-14 shrink-0 place-items-center rounded-2xl text-xl font-black ${
                  (monitoring?.seo_score ?? 0) >= 80
                    ? "bg-emerald-100 text-emerald-700"
                    : (monitoring?.seo_score ?? 0) >= 50
              ...
              <div>
                <p className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#3139fb]/60">
                  SEO Health Score
                </p>
                <p className="font-heading text-lg font-bold text-[#3139fb]">
                  {monitoring?.seo_score !== null &&
                  monitoring?.seo_score !== undefined
                    ? monitoring.seo_score >= 80
                      ? "Great — Well Optimized"
                      : monitoring.seo_score >= 50
                        ? "Good — Some Improvements Needed"
                        : "Poor — Needs Attention"
                    : "Run 'Check Now' to analyze SEO"}
                </p>
                <p className="mt-0.5 text-xs text-[#3139fb]/60">
                  Based on 11 checks across titles, meta, OG, Twitter,
            ...
            {monitoring?.seo_checks && monitoring.seo_checks.length > 0 && (
              <Section
                title="All SEO Checks"
                icon={<Search className="size-4 text-[#3139fb]" />}
              >
                <div className="divide-y divide-border/50">
                  {monitoring.seo_checks.map((check) => (
                    ...
                    >
                      ...
                      {check.detail && (
                        <span className="text-right text-[11px] text-muted-foreground break-all max-w-[55%]">
                          {check.detail}
                        </span>
                      )}
                    </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              >
                ...
                <Row
                  label="Title Length"
                  value={
                    monitoring?.seo_title
                      ? `${monitoring.seo_title.length} chars`
                      : "—"
                  }
                />
                <Row
                  ...
                  value={
                    monitoring?.seo_meta_description
                      ? `${monitoring.seo_meta_description.length} chars`
                      : "Missing"
                  }
                />
                <Row
                  label="Canonical URL"
                  value={monitoring?.seo_canonical_url || "Not set"}
                  mono
                />
                <Row
                  label="Robots Meta"
                  value={
                    monitoring?.seo_robots_meta ||
                    "Not set (default: index,follow)"
                  }
                />
                <Row
                  label="Indexable"
                  value={
                    monitoring?.seo_is_indexable ? "✅ Yes" : "❌ No (noindex)"
                  }
                />
              </Section>
              ...
              >
                <Row
                  label="HTTPS"
                  value={
                    monitoring?.seo_https_redirects
                      ? "✅ Served over HTTPS"
                      : "❌ Not HTTPS"
                  }
                />
                <Row
                  label="www Canonical"
                  value={monitoring?.seo_www_canonical || "—"}
                />
                <Row
                  label="robots.txt"
                  value={
                    monitoring?.seo_robots_txt_exists
                      ? monitoring.seo_robots_txt_blocks_self
                        ? "⚠️ Blocking all crawlers!"
                        : "✅ Found"
                      : "⚠️ Not found"
                  }
                />
                <Row
                  label="Sitemap"
                  value={
                    monitoring?.seo_sitemap_found ? "✅ Found" : "⚠️ Not found"
                  }
                />
                {monitoring?.seo_sitemap_url &&
                  monitoring.seo_sitemap_found && (
                    <Row
                      label="Sitemap URL"
                      value={
                        <a
                          href={monitoring.seo_sitemap_url}
                          target="_blank"
                          rel="noreferrer"
                          className="text-[#3139fb] hover:underline break-all text-[11px]"
                        >
                          {monitoring.seo_sitemap_url}
                        </a>
                      }
              ...
              >
                <Row
                  label="OG Tags Found"
                  value={monitoring?.seo_has_og ? "✅ Yes" : "❌ No"}
                />
                <Row label="OG Title" value={monitoring?.seo_og_title || "—"} />
                <Row
                  label="OG Description"
                  value={
                    monitoring?.seo_og_description
                      ? `${monitoring.seo_og_description.substring(0, 80)}…`
                      : "—"
                  }
                />
                <Row
                  label="OG Image"
                  value={
                    monitoring?.seo_og_image ? (
                      <a
                        href={monitoring.seo_og_image}
                        target="_blank"
                        rel="noreferrer"
                        className="text-[#3139fb] hover:underline break-all text-[11px]"
                      >
              ...
              <Section
                ...
                icon={<AlertTriangle className="size-4 text-sky-500" />}
              >
                <Row
                  label="Card Found"
                  value={
                    monitoring?.seo_has_twitter_card
                      ? "✅ Yes"
                      : "⚠️ Not configured"
                  }
                />
                <Row
                  label="Card Type"
                  value={monitoring?.seo_twitter_card || "—"}
                />
              </Section>
            </div>
          </div>
        ...
        {activeTab === "events" && (
          <Section
            ...
            icon={<RefreshCw className="size-4 text-[#3139fb]" />}
...
function Row({
  ...
  return (
    <div className="flex flex-col gap-1 py-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4 border-b border-border/50 last:border-0">
      ...
      <span
        className={`text-right text-xs font-semibold text-foreground break-all ${mono ? "font-mono text-[11px]" : ""}`}
      >
        {value}
      </span>
    </div>
...
function MiniCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 shadow-xs">
      ...
      <p
        className="font-heading text-sm font-bold text-foreground truncate"
        title={value}
      >
        {value}
      </p>
...
function StatusPill({
  ...
  return (
    ...
    >
      <span
        className={`size-1.5 rounded-full ${ok ? "bg-emerald-500" : warn ? "bg-amber-500" : "bg-red-500"}`}
      />
      {label}
    </span>
  );
      ...
      <div className="space-y-1.5">
        {items && items.length > 0 ? (
          items.map((item) => (
            <div
              key={item}
              className={`flex items-start gap-2 rounded-lg border px-2.5 py-2 font-mono text-xs ${colors[color]}`}
            >
...
function RecordBox({
  ...
  return (
    <div className="mb-2 flex items-start gap-2 rounded-lg bg-muted/50 px-2.5 py-2">
      <span className="flex-1 break-all font-mono text-[10px] text-foreground leading-relaxed">
        {value}
      </span>
      <button
